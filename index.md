<div class="card custom-border text-white bg-transparent">
    <div class="full_wh blur"></div>
    <div class="card-body">
        <div class="container">
            <div class="row">
                <div class="col-sm m-4">
                    <div class="row">
                        <h1>OKTW 星系計畫</h1>
                    </div>
                    <div class="row">
                        <h3>Sample TEXT 文字文字文字文字文字文字文字文字文字文字文字文字文字文字文字文字</h3>
                    </div>
                </div>
                <div class="col-0">
                    <div style="height:100%;width:3px;background:#212529"></div>
                </div>
                <div class="col-sm m-4">
                    <div class="row">
                        <h2>連線位置 :
                            <code>oktw.tw</code>
                        </h2>
                    </div>
                    <div class="row">
                        <h2>遊戲版本 :
                            <code>1.12.2</code>
                        </h2>
                    </div>
                </div>
            </div>
        </div>

        <div class="container mt-5">
            <div class="row">
                <div class="col-sm"></div>
                <div class="col-sm"></div>
                <div class="col-sm text-center">
                    <button type="button" class="btn btn-lg pl-5 pr-5 m-2 btn-secondary">News</button>
                </div>
                <div class="col-sm text-center">
                    <button type="button" class="btn btn-lg pl-5 pr-5 m-2 btn-secondary">Blog</button>
                </div>
                <div class="col-sm"></div>
                <div class="col-sm"></div>
            </div>
        </div>
    </div>
</div>